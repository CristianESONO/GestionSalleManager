...
public class App extends Application {
    ...
    /** Retourne la fenêtre principale (pour minimiser / fermer depuis la barre de titre). */
    ...
    public void start(Stage stage) throws IOException {
        // Avant init BDD : recopier URL / clé paiement QR du JAR vers APPDATA (mise à jour ou nouveau build)
        AppConfig.applyBundledPaymentQrDefaultsToExternalFile();
        ...
        // Icône zone de notification : le bouton Réduire masquera la fenêtre vers le tray
        Platform.runLater(() -> TrayHelper.setupTray(mainStage));
    ...
    /**
     * Compare deux versions (ex. 1.3.4 et 1.3.5). Retourne true si latest est plus récente que current.
     */
    private static boolean isNewerVersion(String latest, String current) {
        ...
        for (int i = 0; i < Math.max(a.length, b.length); i++) {
            int va = i < a.length ? parseIntSafe(a[i], 0) : 0;
            int vb = i < b.length ? parseIntSafe(b[i], 0) : 0;
    ...
    private static void onVersionCheckDone(String lastVersion, boolean fromMenu) {
        ...
        if (lastVersion != null && isNewerVersion(lastVersion, currentVersion)) {
