...
public class UserController {
    ...
    @FXML
    private Button addClientButton, editClientButton, deleteClientButton;
    @FXML
    ...
    @FXML
    private Button btnVoirHistorique, btnGenererPdfHistorique, editCompteClientButton, deleteCompteClientButton;
    @FXML
    ...
    // v1.3.5 : pagination -> liste sans fin (on désactive la pagination visuelle)
    private static final boolean INFINITE_LIST = true;
        ...
        actionsRemainingTimeColumn.setCellFactory(param -> new TableCell<Client, Void>() {
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty) {
                    ...
                    Client client = getTableView().getItems().get(getIndex());
                    Duration remainingTime = calculateRemainingTimeForClient(client);
                    boolean hasRemainingTime = remainingTime != null && !remainingTime.isZero() && !remainingTime.isNegative();
                    restartSessionButton.setVisible(hasRemainingTime);
                    restartSessionButton.setManaged(hasRemainingTime);
    ...
    private void setupPagination() {
        if (INFINITE_LIST) {
            if (paginationClient != null) { paginationClient.setVisible(false); paginationClient.setManaged(false); paginationClient.setPageCount(1); }
            if (paginationAdmin != null) { paginationAdmin.setVisible(false); paginationAdmin.setManaged(false); paginationAdmin.setPageCount(1); }
            if (paginationParrain != null) { paginationParrain.setVisible(false); paginationParrain.setManaged(false); paginationParrain.setPageCount(1); }
            if (paginationRemainingTime != null) { paginationRemainingTime.setVisible(false); paginationRemainingTime.setManaged(false); paginationRemainingTime.setPageCount(1); }
            // Afficher directement les listes complètes (filtrées)
            userTable.setItems(displayedUsers);
            clientTable.setItems(displayedClients);
            parrainTable.setItems(displayedParrains);
            remainingTimeClientsTable.setItems(displayedRemainingTimeClients);
            return;
        }
    ...
    private void updatePaginationPageCount(Pagination pagination, int totalItems) {
        if (INFINITE_LIST || pagination == null) {
            return;
        }
    ...
    private ObservableList<User> getUsersPage(int pageIndex) {
        if (INFINITE_LIST) return displayedUsers;
    ...
    private ObservableList<Client> getClientsPage(int pageIndex) {
        if (INFINITE_LIST) return displayedClients;
        int fromIndex = pageIndex * PAGE_SIZE;
    ...
    private ObservableList<Parrain> getParrainsPage(int pageIndex) {
        if (INFINITE_LIST) return displayedParrains;
        int fromIndex = pageIndex * PAGE_SIZE;
    ...
    private ObservableList<Client> getRemainingTimeClientsPage(int pageIndex) {
        if (INFINITE_LIST) return displayedRemainingTimeClients;
        int fromIndex = pageIndex * PAGE_SIZE;
    ...
    private void managePermissions() {
        ...
        // SuperAdmin : pas de boutons Ajouter/Modifier/Supprimer dans Clients fidèles ; Modifier/Supprimer dans Compte client uniquement
        boolean canAddClient = isAdmin && !isSuperAdmin;
        if (addClientButton != null) { addClientButton.setVisible(canAddClient); addClientButton.setManaged(canAddClient); }
        if (editClientButton != null) { editClientButton.setVisible(false); editClientButton.setManaged(false); }
        if (deleteClientButton != null) { deleteClientButton.setVisible(false); deleteClientButton.setManaged(false); }
        if (editCompteClientButton != null) { editCompteClientButton.setVisible(isSuperAdmin); editCompteClientButton.setManaged(isSuperAdmin); }
        if (deleteCompteClientButton != null) { deleteCompteClientButton.setVisible(isSuperAdmin); deleteCompteClientButton.setManaged(isSuperAdmin); }
    ...
    /** Modifier le compte client sélectionné (onglet Compte client) — réservé au SuperAdmin. */
    @FXML
    private void openEditCompteClientWindow() {
        if (ConnexionController.user == null || !ConnexionController.user.getRole().equals(Role.SuperAdmin)) {
            showAlert(AlertType.WARNING, "Accès refusé : réservé au SuperAdmin.");
            return;
        }
        Client selected = remainingTimeClientsTable != null ? remainingTimeClientsTable.getSelectionModel().getSelectedItem() : null;
        if (selected == null) {
            showAlert(AlertType.WARNING, "Veuillez sélectionner un client dans la liste Compte client.");
            return;
        }
        openModalWindow("/com/views/EditClientWindow.fxml", "Modifier le compte client", selected);
    }

    /** Supprimer le compte client sélectionné (onglet Compte client) — réservé au SuperAdmin. */
    @FXML
    private void deleteCompteClient() {
        if (ConnexionController.user == null || !ConnexionController.user.getRole().equals(Role.SuperAdmin)) {
            showAlert(AlertType.WARNING, "Accès refusé : réservé au SuperAdmin.");
            return;
        }
        ...
        Duration remainingTime = calculateRemainingTimeForClient(selected);
        if (remainingTime != null && remainingTime.toMinutes() > 0) {
            showAlert(AlertType.WARNING, "Suppression impossible : ce client a encore du temps restant (" + formatDuration(remainingTime) + ").");
            return;
        }
        Alert confirm = new Alert(AlertType.CONFIRMATION, "Voulez-vous vraiment supprimer ce compte client ?", ButtonType.YES, ButtonType.NO);
        confirm.showAndWait().ifPresent(response -> {
            if (response == ButtonType.YES) {
                try {
                    Fabrique.getService().deleteClient(selected);
                    showAlert(AlertType.INFORMATION, "Compte client supprimé avec succès.");
                    refreshAllData();
                } catch (Exception e) {
                    showAlert(AlertType.ERROR, "Erreur lors de la suppression : " + e.getMessage());
                }
            }
        });
    }
    ...
    private void deleteClient() {
        if (ConnexionController.user != null && ConnexionController.user.getRole().equals(Role.SuperAdmin)) {
            ...
            // v1.3.5 : impossible de supprimer un compte client avec des minutes restantes
            Duration remainingTime = calculateRemainingTimeForClient(selectedClient);
            if (remainingTime != null && remainingTime.toMinutes() > 0) {
                showAlert(AlertType.WARNING, "Suppression impossible : ce client a encore du temps restant (" + formatDuration(remainingTime) + ").");
                return;
            }
   ...
   private void restartSessionWithRemainingTime(Client client) {
    ...
    /** v1.3.5 : ouvrir la fenêtre Historique du client sélectionné (onglet Compte client). */
    @FXML
    private void openClientHistoryWindow() {
        Client selected = remainingTimeClientsTable != null ? remainingTimeClientsTable.getSelectionModel().getSelectedItem() : null;
        if (selected == null) {
            showAlert(AlertType.WARNING, "Veuillez sélectionner un client dans la liste « Compte client ».");
            return;
        ...
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/views/ClientHistoryWindow.fxml"));
            javafx.scene.Scene scene = new javafx.scene.Scene(loader.load());
            scene.getStylesheets().add(getClass().getResource("/com/css/style.css").toExternalForm());
            Stage stage = new Stage();
            stage.setTitle("Historique — " + (selected.getName() != null ? selected.getName() : "Client"));
            stage.setScene(scene);
            stage.initModality(Modality.APPLICATION_MODAL);
            ClientHistoryController ctrl = loader.getController();
            ctrl.setClient(selected);
            stage.showAndWait();
        } catch (IOException e) {
            showAlert(AlertType.ERROR, "Impossible d'ouvrir l'historique : " + e.getMessage());
        }
    }

    /** v1.3.5 : générer le PDF historique du client sélectionné (onglet Compte client). */
    @FXML
    private void generateClientHistoryPdf() {
        Client selected = remainingTimeClientsTable != null ? remainingTimeClientsTable.getSelectionModel().getSelectedItem() : null;
        if (selected == null) {
            showAlert(AlertType.WARNING, "Veuillez sélectionner un client dans la liste « Compte client ».");
            return;
        }
        java.util.List<Reservation> reservations = Fabrique.getService().findReservationsByClientIdWithRelations(selected.getId());
        if (reservations == null) reservations = java.util.List.of();
        ...
        String safeName = (selected.getName() != null ? selected.getName().replaceAll("[^a-zA-Z0-9]", "_") : "client") + "_historique.pdf";
        fc.setInitialFileName(safeName);
        java.io.File file = fc.showSaveDialog(remainingTimeClientsTable.getScene().getWindow());
        if (file == null) return;
        try {
            ClientHistoryController.genererPdfLandscape(selected, reservations, file);
            showAlert(AlertType.INFORMATION, "PDF enregistré : " + file.getAbsolutePath());
        } catch (Exception e) {
            showAlert(AlertType.ERROR, "Erreur PDF : " + (e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName()));
        }
    }
}