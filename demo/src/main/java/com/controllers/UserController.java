...
public class UserController {
    ...
    private void managePermissions() {
        ...
        // v1.3.5 : en admin, afficher seulement "Ajouter un client"
        boolean canAddClient = isSuperAdmin || isAdmin;
        if (addClientButton != null) { addClientButton.setVisible(canAddClient); addClientButton.setManaged(canAddClient); }
        if (editClientButton != null) { editClientButton.setVisible(isSuperAdmin); editClientButton.setManaged(isSuperAdmin); }
        if (deleteClientButton != null) { deleteClientButton.setVisible(isSuperAdmin); deleteClientButton.setManaged(isSuperAdmin); }
   ...
   private void restartSessionWithRemainingTime(Client client) {
    ...
    private void generateClientHistoryPdf() {
        ...
        List<GameSession> sessions = Fabrique.getService().findGameSessionsByClientIdWithRelations(selected.getId());
        ...
        if (file == null) return;
        try {
            ClientHistoryController.genererPdfLandscape(selected, sessions, file);
