...
private void promptForExtension(GameSession session) {
    ...
    // Traitement du résultat : prolongation validée
    result.ifPresent(pair -> {
        try {
            ...
            refreshViewsAndClose();
        } catch (Exception e) {
            ControllerUtils.showErrorAlert("Erreur", "Échec de la prolongation : " + e.getMessage());
        }
    });

    // Si l'utilisateur a cliqué "Annuler" sur le dialogue de prolongation : fermer et rafraîchir
    if (!result.isPresent()) {
        refreshViewsAndClose();
    }
}

private void refreshViewsAndClose() {
    if (reservationController != null) {
        reservationController.loadReservations();
        reservationController.loadActiveSessions();
    }
    if (PosteJeuController.getInstance() != null) {
        try {
            PosteJeuController.getInstance().refreshPostes();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
