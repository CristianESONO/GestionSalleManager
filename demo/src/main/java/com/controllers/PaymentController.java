...
import com.utils.MobileMoneyQrConfirmDialog;
...
public class PaymentController {
    ...
    private void handleValider() throws Exception {
        ...
        try {
            if (requiresMobileMoneyQrStep(modePaiement)) {
                Window owner = modePaiementComboBox.getScene() != null ? modePaiementComboBox.getScene().getWindow() : null;
                if (!MobileMoneyQrConfirmDialog.showAndWait(owner, modePaiement, montantTotal)) {
                    return;
                }
            }
            ...
            stage.close();
            showAlert("Paiement réussi", "La transaction a été enregistrée.");
        } catch (Exception e) {
            e.printStackTrace();
