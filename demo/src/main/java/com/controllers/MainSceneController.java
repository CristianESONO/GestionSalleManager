...
public class MainSceneController implements Initializable {
    ...
    // Méthodes de contrôle de la fenêtre
    @FXML private void handleMinimize() {
        Stage stage = App.getMainStage();
        if (stage == null) return;
        // Minimiser = masquer vers la zone de notification (icône dans la barre des tâches)
        if (com.utils.TrayHelper.isTrayAvailable()) {
            stage.hide();
        } else {
    ...
    public void handleDeconnexion() {
        ...
        if (option.isPresent() && option.get().equals(ButtonType.OK)) {
            Fabrique.getService().setCurrentUser(null);
            loadView("connexion");
    ...
    public void handleBackup() {
        Path saved = BackupService.backupNow();
        Alert alert = new Alert(AlertType.INFORMATION);
        alert.setTitle("Sauvegarde");
        if (saved != null) {
            alert.setHeaderText("Sauvegarde effectuée");
            alert.setContentText("La base a été sauvegardée dans :\n" + saved.toAbsolutePath());
        } else {
            alert.setHeaderText("Échec de la sauvegarde");
            alert.setContentText("Impossible de créer la sauvegarde. Consultez les logs.");
        }
        alert.showAndWait();
    }
