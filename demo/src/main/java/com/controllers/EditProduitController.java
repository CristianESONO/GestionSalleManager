...
public class EditProduitController {
    ...
    public void setProduit(Produit produit) {
        // Toujours recharger depuis la base : la grille peut garder une instance périmée ;
        // un merge Hibernate réécrirait tout l'état (dont le stock) et peut « annuler » des ventes.
        Produit fromDb = Fabrique.getService().findProduitById(produit.getId());
        this.produit = fromDb != null ? fromDb : produit;

        nomField.setText(this.produit.getNom());
        prixField.setText(this.produit.getPrix().toPlainString());
        stockField.setText(String.valueOf(this.produit.getStock()));
        dateLimitePicker.setValue(this.produit.getDateLimiteConsommation());

        // Charger et afficher l'image existante (résolution du chemin APPDATA ou absolu)
        String existingImagePath = this.produit.getImage();
        ...
        if (categorieComboBox != null) {
            ...
            categorieComboBox.setValue(this.produit.getCategorie());
        }
    }
    ...
    private void updateProduit(ActionEvent event) {
        ...
        Produit toSave = Fabrique.getService().findProduitById(produit.getId());
        if (toSave == null) {
            ControllerUtils.showErrorAlert("Erreur", "Ce produit n'existe plus en base.");
            return;
        }
        ...
        try {
            ...
            String finalImagePath = toSave.getImage();
            ...
            if (sourceFile != null && sourceFile.exists() && !currentTxtImagePath.equals(toSave.getImage())) {
                try {
            ...
            } else if (currentTxtImagePath.isEmpty() && (toSave.getImage() == null || toSave.getImage().isEmpty())) {
                finalImagePath = null;
            }
            // Sinon : on garde l'image déjà enregistrée (finalImagePath)

            // Mise à jour sur l'entité alignée avec la base (évite merge d'un détaché périmé)
            toSave.setNom(nom);
            toSave.setPrix(prix);
            toSave.setStock(stock);
            toSave.setImage(finalImagePath);
            toSave.setDateLimiteConsommation(dateLimite);
            if (categorieComboBox != null) {
                toSave.setCategorie(categorieComboBox.getValue());
            }

            Fabrique.getService().updateProduit(toSave);
