    ...
    public List<GameSession> findGameSessionsByClientIdWithRelations(int clientId) {
        ...
        try {
            TypedQuery<GameSession> query = em.createQuery(
                ...
                "LEFT JOIN FETCH gs.game " +
                "LEFT JOIN FETCH gs.poste " +
                "LEFT JOIN FETCH gs.reservation " +
                "WHERE gs.client.id = :clientId ORDER BY gs.startTime DESC", GameSession.class);
            query.setParameter("clientId", clientId);
            gameSessions = query.getResultList();
        } catch (Exception e) {
            System.err.println("Erreur findGameSessionsByClientIdWithRelations: " + e.getMessage());
        } finally {
            em.close();
        }
        return gameSessions != null ? gameSessions : new java.util.ArrayList<>();
    }
