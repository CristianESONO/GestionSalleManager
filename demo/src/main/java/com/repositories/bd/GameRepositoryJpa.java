...
public class GameRepositoryJpa implements IGameRepository {
    ...
    public void delete(int id) {
        ...
        try {
            transaction = em.getTransaction();
            transaction.begin();
            List<Game> result = em.createQuery("SELECT g FROM Game g LEFT JOIN FETCH g.postes WHERE g.id = :id", Game.class)
                .setParameter("id", id)
                .getResultList();
            Game game = result.isEmpty() ? null : result.get(0);
            if (game != null) {
                // Retirer le jeu de tous les postes auxquels il est associé (table poste_game)
                if (game.getPostes() != null && !game.getPostes().isEmpty()) {
                    List<Poste> postes = new java.util.ArrayList<>(game.getPostes());
                    for (Poste p : postes) {
                        if (p.getGames() != null) {
                            p.getGames().removeIf(g -> g != null && g.getId() == id);
                            em.merge(p);
                        }
                    }
                }
