...
public class ReservationRepositoryJpa implements IReservationRepository {
    ...
    @Override
    public List<Reservation> findByClientIdWithRelations(int clientId) {
        EntityManager em = JpaUtil.getEntityManager();
        List<Reservation> reservations = null;
        try {
            TypedQuery<Reservation> query = em.createQuery(
                "SELECT r FROM Reservation r " +
                "LEFT JOIN FETCH r.poste LEFT JOIN FETCH r.game " +
                "WHERE r.client.id = :clientId ORDER BY r.reservationDate DESC", Reservation.class);
            query.setParameter("clientId", clientId);
            reservations = query.getResultList();
        } catch (Exception e) {
            System.err.println("Erreur findByClientIdWithRelations: " + e.getMessage());
            e.printStackTrace();
        } finally {
            em.close();
        }
        return reservations != null ? reservations : java.util.Collections.emptyList();
    }
